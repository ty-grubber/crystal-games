...
<Dialog bind:open={isOpen} slot="over" surface$style="height: 700px">
  ...
  <Content id="howToContent">
    ...
    <p>
      This tracker works for use with most versions of the Item Randomizer and has been tested to work up to v8.1.2.
    </p>
    ...
    <ul>
      ...
      <li><b>5 Points:</b> Any other Key Item that unlocks additional checks, like Rock Smash or Basement Key</li>
    ...
    <p>
      The Points Hint Tracker also has the ability to connect multiple players together without the need of sharing a spoiler file. The Host connects to the game initially, setting up the tracker with the above options. Once connected, other players can connect to the Host's game using the Host ID generated by the Host (see below)
    </p>

    <p>
      There are some additional options to configure when setting up a game to host:
    </p>
    <ul>
      <li>
      ...
      <li>
        <b>Host Game As Spectator:</b> If you plan to spectate this game instead of playing with the players, check this box off. If checked, you will get a specific <Button color="primary" on:click={() => spectatorTrackerDialogOpen = true}>Spectator Tracker</Button> that lets you see every player's progress as they play.
      </li>
      <li>
        <b>Game Name:</b> The name of the game being hosted. This is public to all players when they connect and should be used to give the players a reminder of the modifiers being used (ex. Max FIR Shopsanity Race)
      </li>
      <li>
        <b>Private Host ID:</b> The Host ID used to connect players to your game. This is a generated value that cannot be manually changed or edited. This should only be shared with your players so unwanted players do not enter your game.
      </li>
    </ul>

    <p>
      Players will not be able to connect with your hosted game until you have clicked the Start Game button in the modal. It is advisable to share the Host ID after you have started the game so connections are smooth.
    </p>

    <p>
      If you are a player joining a hosted game, your options are much more limited (as the host will provide you with all necessary info to run your seed):
    </p>
    <ul>
      <li>
      ...
      <li>
        <b>Tracker Layout:</b> You still get to select the tracker of your preference. All trackers work with connected games.
      </li>
    </ul>

    <p>
      As a player, make sure you do not attempt to connect to the Host until the Host has started the game. You'll know the connection is successful if, at the bottom of the tracker, you see an area showing the Game Name, Host Name (if the host is a spectator), and the Player List. This area will update as more players connect.
    </p>
  </Content>
  <Actions>
    <Button>Close</Button>
...
<Dialog bind:open={spectatorTrackerDialogOpen} slot="over">
  ...
  <Content>
    <p>
      Upon initializing the game, you'll be presented with a mostly blank screen showing the game name and a message saying that the game is waiting for players to join. Once any player connects to your game, the tracker will show. This tracker is a read-only tracker that is designed to show you all of the progress each connected player has made within their own tracker.
    </p>
    <p>
      This tracker has certain functionality that is the same as the Compact tracker and has the familiarity of the item order of the EmoTracker for Pokémon Crystal. This includes:
    </p>
    <ul>
      <li>Key Items showing in roughly the same layout as the EmoTracker, including lighting up when they have been found</li>
      <li>Region point values getting revealed as 9-pt items are placed and decreasing as items are placed within them</li>
      <li>The most recent region that was revealed by the player has its point value underlined</li>
    </ul>

    <p>
      There are a number of notable functionality differences, too.
    </p>
    <ul>
      <li>All regions are shrunk to just show their icon and how many points are left in the region (which is ?? if not known)</li>
      <li>A region lit with a green background is the current focussed region. The current focussed region shows all of its region details in a green-bordered box under the tracker. This section design and data mimics the design and data of the Compact Tracker (ie. it has region icon, points left, items found, and points found)</li>
      <li>Clicking any region in the tracker makes it become the current focussed region</li>
      <li>Clicking any found item in the Item section will immediately cause the region that item was found in, to be the current focussed region.</li>
    </ul>

    <p>
      This tracker is designed to only show two players at a time. If your game contains more than 2 players, a dropdown will appear under the trackers for each additional player connected to your game. Each dropdown has the following functionality:
    </p>
    <ul>
      <li>The dropdown shows the player's name, how many points they have left to find, and how many Revealed Regions are on their tracker (denoted with "RR")</li>
      <li>Clicking the dropdown offer the option to swap that player into the left tracker or the right tracker on the main view. The player being replaced will be represented by this dropdown.</li>
    </ul>
  </Content>
  <Actions>
    <Button>Close</Button>
